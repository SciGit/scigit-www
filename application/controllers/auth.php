...
class Auth extends SciGit_Site_Controller
{
	...
	function index()
	{
		if ($message = $this->session->flashdata('message')) {
			$this->twig->display('auth/general_message.twig',
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->twig->display('auth/login_form.twig', $data);
		}
	}
  ...
  function registered()
  {
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
          ...
          $this->project->create($data['user_id'],
            ...
            $this->email_queue->add_user_email($data['user_id']);
			...
			$this->twig->display('auth/register_form.twig', $data);
		}
	}
	...
	function send_again()
	{
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			$this->twig->display('auth/send_again_form.twig', $data);
		}
	}
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
          ...
          email_forgot_password($data);
			...
			$this->twig->display('auth/forgot_password_form.twig', $data);
		}
	}
	...
	function reset_password()
	{
		...
		$this->twig->display('auth/reset_password_form.twig', $data);
	}
	...
	function change_password()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->twig->display('auth/change_password_form.twig', $data);
		}
	}
    ...
    show_404(); // don't use this right now
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->twig->display('auth/change_email_form.twig', $data);
		}
	}
	...
	function unregister()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->twig->display('auth/unregister_form.twig', $data);
		}
	}
