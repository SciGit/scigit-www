...
(function() {
  window.ajaxFormSubmit = function(container, form) {
    var alertError, alertSuccess, btnCancel, btnSubmit, btnSubmitText, formData, parseErrorResponseText, parseResponseText, parseSuccessResponseText;

    parseSuccessResponseText = function(data) {
      var spinner;

      spinner = '<i class="icon-spin icon-spinner"></i>';
      if (data.url != null) {
        setTimeout(function() {
          return Turbolinks.visit(data.url);
        }, 2000);
    ...
    parseErrorResponseText = function(data) {
      var e, error, field, list, response, responseText;
    ...
    parseResponseText = function(data, response) {
      if (response === 'success') {
        return parseSuccessResponseText(data);
      } else {
        return parseErrorResponseText(data);
      }
    };
    ...
    container.on('click', '.btnSubmit, .btnCancel', function() {
    ...
    return $.ajax({
      ...
      complete: function(data) {
        ...
        return container.off('click', '.btnSubmit, .btnCancel');
      },
...
(function() {
  this.DiffViewer = (function() {
    function DiffViewer(change_id, project_id, commit_hash) {
      var _this = this;

      this.change_id = change_id;
      this.project_id = project_id;
      this.commit_hash = commit_hash;
      this.files = $("#file-listing-" + this.change_id);
      this.viewerPane = $('#diff-viewer');
      $.ajax({
        url: "/projects/" + this.project_id + "/changes/" + this.change_id + "/diff.json",
        type: "GET",
        cache: false,
        success: function(json) {
          return _this.displayDiff(json);
        },
        error: function(a, b, httpError) {
          return _this.displayError(httpError);
        }
      });
      this.files.html('<li class="nav-header">' + '<i class="icon-spin icon-spinner"></i>&nbsp;&nbsp;Loading...</li>');
      this.viewerPane.html('<center><i class="icon-spin icon-spinner icon-3x"></i></center>');
    }

    DiffViewer.prototype.displayDiff = function(json) {
      var div, _i, _len, _ref, _results;

      if ((json.files_html == null) || (json.diff_viewer_html == null)) {
        this.displayError("Invalid response");
        return;
      }
      this.files.html(json.files_html);
      this.viewerPane.html(json.diff_viewer_html);
      _ref = this.viewerPane.find('.diff-file');
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        div = _ref[_i];
        _results.push((function(elem) {
          elem.on('mouseover', 'span.modify, td.modify', function() {
            var match;

            match = $(this).attr('class').match(/modify-[0-9]+/);
            return (match != null) && $('.' + match[0]).addClass('hover');
          });
          elem.on('mouseout', 'span.modify, td.modify', function() {
            var match;

            match = $(this).attr('class').match(/modify-[0-9]+/);
            return (match != null) && $('.' + match[0]).removeClass('hover');
          });
          elem.on('click', '.btn.inline', function() {
            elem.find('tbody.inline').removeClass('hidden');
            elem.find('tbody.side').addClass('hidden');
            elem.find('.btn.inline').addClass('active');
            return elem.find('.btn.side').removeClass('active');
          });
          return elem.on('click', '.btn.side', function() {
            elem.find('tbody.inline').addClass('hidden');
            elem.find('tbody.side').removeClass('hidden');
            elem.find('.btn.inline').removeClass('active');
            return elem.find('.btn.side').addClass('active');
          });
        })($(div)));
      }
      return _results;
    ...
    DiffViewer.prototype.displayError = function(error) {
      var error_msg;

      this.files.html('');
      error_msg = "Error loading the specified change. Please try again later. (" + error + ")";
      return this.viewerPane.html("<div class='alert alert-error'>" + error_msg + "</div>");
    };
...
(function() {
  ...
  $(document).on('ready page:load', function() {
    $('#label-projects').click(function(e) {
      if (e.target === this || e.target.tagName.toLowerCase === 'div') {
        return Turbolinks.visit(this.href);
      }
    });
    $('.project').click(function(e) {
      if (e.target === this || e.target.tagName.toLowerCase === 'div') {
        return Turbolinks.visit('/projects/' + $(this).data('proj_id'));
      }
    });
    $('body').on('click', '.change', function(e) {
      var change_id, project_id;

      if (e.target === this || e.target.tagName.toLowerCase === 'div') {
        project_id = $(this).data('proj_id');
        change_id = $(this).data('change_id');
        return Turbolinks.visit("/projects/" + project_id + "/changes/" + change_id + "/");
      }
    });
    $(window).on('hashchange', function() {
      return scrollBy(0, -50);
    });
    return $('body').on('click', '.hash-anchor', function(e) {
...
(function() {
  var loadDiff, project_id, resetActives, toggleActive;
  ...
  resetActives = function() {
    ...
    return $('.action button i').addClass('icon-plus');
  };

  loadDiff = function(selected) {
    var change_id, commit_hash;
    ...
    return new DiffViewer(change_id, project_id, commit_hash);
  };
  ...
  $(document).on('ready page:load', function() {
    ...
    if (change.length > 0) {
      ...
      if (cl.length > 0) {
        cl.affix({
          offset: {
            top: cl.position().top - 60
          }
        });
        cl.on('click', 'li.change-list-item', function(e) {
        ...
        return cl.on('click', '.action button', function(e) {
          var icon;

          icon = $(this).find('i');
          icon.toggleClass('icon-plus');
          return icon.toggleClass('icon-minus');
        });
      }
    }
  });
...
(function() {
  var hookSubmitAddMember, hookTypeaheadForMemberAdd;

  hookSubmitAddMember = function() {
    var submitAddMemberForm;

    submitAddMemberForm = function(e) {
      e.preventDefault();
      window.ajaxFormSubmit($('#addMemberModal'), $('#addMemberForm'));
      return false;
    };
    $('#addMemberModal').on('submit', '#addMemberForm', submitAddMemberForm);
    return $('#addMemberModal .btnSubmit').click(submitAddMemberForm);
  };

  hookTypeaheadForMemberAdd = function() {
    return $('#addMemberModal').on('shown', function() {
      var checkIfQueryIsValidEmail, closePopovers, findResultWithLabel, hookPermissionButtonClick, indicateButton, indicateFound, indicateLoading, indicateNotFound, indicateSearch, previousResults, styleButtonAsAddMember, styleButtonAsInvite;

      checkIfQueryIsValidEmail = function() {
        var regex;
      ...
      indicateButton = function(btnClass, iconClass, showPopover) {
        if (showPopover == null) {
          showPopover = null;
        }
        ...
        if (showPopover === true) {
          ...
          return $('#project_permission_user_attributes_email').focus();
        } else if (showPopover === false) {
          return $('#findMember').popover('hide');
        }
      };
      indicateSearch = function() {
        return indicateButton('btn-info', 'icon-search', false);
      };
      indicateLoading = function() {
        return indicateButton('btn-primary', 'icon-spin icon-spinner');
      };
      indicateNotFound = function(showPopover) {
        return indicateButton('btn-danger', 'icon-remove', showPopover);
      };
      indicateFound = function() {
        return indicateButton('btn-success', 'icon-ok', false);
      };
      closePopovers = function(excludeFindMember) {
        if (excludeFindMember == null) {
          excludeFindMember = false;
        }
        if (!excludeFindMember) {
          $('#findMember').popover('hide');
        }
        $('#invalidEmailPopover').popover('hide');
        $('#permissionHeader').popover('hide');
        return $('#addMemberModal .btnSubmit').popover('hide');
      };
      styleButtonAsAddMember = function() {
        closePopovers();
        return $('#addMemberModal .btnSubmit').removeClass('btn-success btn-danger disabled').addClass('btn-primary').html('Add Member').prop('disabled', false).data('invite', false);
      };
      styleButtonAsInvite = function() {
        var button;
        ...
        if ($('input[name="project_permission[permission]"]:checked').val() > 0 && checkIfQueryIsValidEmail()) {
          return button.removeClass('btn-danger disabled').addClass('btn-success').html('Invite').prop('disabled', false);
        } else {
          return button.removeClass('btn-success').addClass('btn-danger disabled').html('<i class="icon-remove"></i> Invite').prop('disabled', 'disabled');
        }
      };
      previousResults = [];
      findResultWithLabel = function(label) {
        var result, _;

        for (_ in previousResults) {
          result = previousResults[_];
      ...
      hookPermissionButtonClick = function() {
        $('#invalidEmailPopover').popover('hide');
        if ($('input[name="project_permission[permission]"]:checked').val() > 0) {
          return $('#addMemberModal .btnSubmit').popover('show');
        } else {
          $('#permissionHeader').popover('show');
          return $('.permission .btn').click(function(e) {
      ...
      $('#addMemberModal').on('click', '#btnInviteMember', function(e) {
        $('#findMember').popover('hide');
        styleButtonAsInvite();
        $('#project_permission_user_attributes_email').focus();
        if (!checkIfQueryIsValidEmail()) {
          return $('#invalidEmailPopover').popover('show');
        }
        indicateFound();
        if ($('input[name="project_permission[permission]"]:checked').val() > 0) {
          return $('#new_project_permission').submit();
        } else {
          return hookPermissionButtonClick();
        }
      });
      $('.permission .btn').click(function(e) {
        ...
        $(this).parent('.permission').removeClass('faded');
        return $(this).find('input[type="radio"]').prop('checked', 'checked');
      });
      $('#addMemberModal').on('keypress', '#project_permission_user_attributes_email', indicateLoading);
      return $('#project_permission_user_attributes_email').typeahead({
        source: function(query, process) {
          var results;

          results = [];
          return $.ajax({
            url: $(this.$element[0]).data('target'),
            type: 'GET',
            dataType: 'json',
            data: {
              term: this.query
  ...
  $(document).on('ready page:load', function() {
    ...
    return hookSubmitAddMember();
  });

}).call(this);
(function() {
  var fetchAndAppendChanges, fetchingChanges, hookCreateNewProjectPublicAndPrivateButtons, hookSubmitCreateNewProject, initInfiniteScrollHelper, loadProjectId, project_id;

  project_id = null;

  fetchingChanges = false;

  hookSubmitCreateNewProject = function() {
    var submitCreateNewProject;

    submitCreateNewProject = function(e) {
      e.preventDefault();
    ...
    return $('#createNewProjectModal .btnSubmit').click(submitCreateNewProject);
  };

  hookCreateNewProjectPublicAndPrivateButtons = function() {
    return $('#createNewProjectModal').on('click', '#btnPrivate, #btnPublic', function(e) {
      var projectPublic;

      e.preventDefault();
      projectPublic = $(this).attr('id') === 'btnPublic';
      if (!projectPublic) {
      ...
      return $("input[id='hiddenPublic']").prop('checked', projectPublic);
    });
  };

  fetchAndAppendChanges = function(page) {
    ...
    return $.ajax('/projects/' + project_id + '/changes/page/' + page, {
      type: 'GET',
      success: function(data) {
        if (!data) {
  ...
  loadProjectId = function() {
    return project_id = $('#project_id').data('project_id');
  };

  initInfiniteScrollHelper = function() {
    return $('body').infiniteScrollHelper({
      loadMore: function(page) {
        return fetchAndAppendChanges(page);
      },
  ...
  $(document).on('ready page:load', function() {
    ...
    if (loadProjectId()) {
      ...
      if (project_id) {
        return fetchAndAppendChanges(1);
      }
    }
  });
